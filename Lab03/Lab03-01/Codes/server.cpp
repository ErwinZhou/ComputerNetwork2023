	...
	*/
	...
	while (true) {
		...
		if (cks == 0 && (recv_header.get_flag() & SYN)) {
			...
			*/
			// 生成随机数
			int randomNumber = rand() % 1; //确保数字在0-1范围内，一定丢包！
		...
		*/
		while (true) {
			...
			while (true) {
				...
				cout << "(An absolute path, or a path relative to D:\\Visual Studio 2022 Code\\Project-Computer Network\\Lab03-01-UDP RDT server)" << endl;
				cin >> output_path;
