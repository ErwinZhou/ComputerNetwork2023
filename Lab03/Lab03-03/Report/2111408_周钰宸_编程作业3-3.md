...
* **服务器端（接收方）：**
  ...
  * **==发送线程：DWORD WINAPI send_thread_main(LPVOID lpParameter)==前面提到过，为了保证效率新加入的发送线程，主要工作有以下：**
    ...
    * 最外层还是大的while(true)，**receive_over等于true并且ack队列为空才退出。**
...
2. **最后一段的ACK丢失问题**：在上次实验Lab03-02中我就提到过，由于最后一个数据报上带有ACK，因此如果接收方（服务器端）收到了最后的数据报就去output文件了，但是回复的ACK丢失了，发送方会在那一直超时重传。

   甚至可以是乱序提前收到最后一个带LAS的数据报并缓存，最后收到的是一个接收窗口中最老的数据报，然后一口气把所有数据报交付给上层后。接收窗口中最老的数据报的ACK发生了丢失。**实际上这种情况在本次SR中发生的更多**，这样也会造成发送方去不停超时重传。

   ==**不过实际上本次实验被我找到了解决方案**==：由于我设置的SR接收方回复ACK的机制是大于receive_end也回复ACK，因此在接收方ouputfile结束后，只需要重新输入丢包率和大小等，**回到“等待接收文件或挥手”的地方（即重新开启全部三个线程）**。此时就可以收到发送方不断重发的上次没收到ACK的数据报，并给它们回复对应的ACK（即使这些数据包序列号可能远大于接收方如今重开后的receive_end）。**因此就迎刃而解了，接下来发送方就可以正常结束上一轮，然后选择挥手或者继续发文件。**

   由此可以看出，**在接收方和发送方窗口大小一致情况下我采用的接收方回复ACK的机制是大于receive_end也回复ACK的方法，加上我Keep-Alive的机制，这样的问题也被解决了。**

   <img src="img/image-20231215100412962.png" alt="image-20231215100412962" style="zoom:50%;" />

   如上图所示，是我设置接收方ACK丢包率高达50%（就是为了让那最后几个ACK丢失），延时为0的测试结果。==**可以看到只要我接收方因为Keep-Alive不用挥手直接进入到下次传输，就可以轻松解决问题，结束死锁，解决上次传输发送方不断超时重传的问题。甚至可以继续下次传输。**==

   不过就像我上次说的，其实即使就算发送方没法正常结束一直在重发，但是接收方肯定已经正确收到了所有的数据报，并且可以output文件了，理论上这次传输已经算是结束了。不过没法挥手结束或者发送方显示一些时延等。**因此无伤大雅。**
...
1. 由于**本次实验要求中明确说明发送方和接收方的窗口大小相同**，因此我在处理接收方接收到序号大于receive_end的数据报时候也采用的是回复ACK的方式，实际上如果双方窗口大小不一致时，遇到这种情况不应该回复ACK。**不过对于本次实验来说足够了。有待改进为只对数据报小于等于receive_end的数据报回复ACK。**不过这样又会带来之前说的最后几个的ACK包丢失可能带来的发送方不停超时重传的情况，**因此仍有待改进。**
